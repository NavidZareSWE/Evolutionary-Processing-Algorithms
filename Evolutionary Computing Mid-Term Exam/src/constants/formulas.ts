...
export const FORMULAS: Record<string, Formula> = {
  expectedCopiesFPS: {
    ...
    plainEnglish:
      "The expected number of copies of an individual equals its fitness divided by the average fitness, times the population size. If your fitness equals the average, you expect 1 copy. If twice the average, expect 2 copies.",
    whenToUse:
      "When analyzing fitness-proportionate selection behavior or comparing individual fitness to population average.",
    example:
      "If population has average fitness 50, individual with fitness 100 expects 2 copies, individual with fitness 25 expects 0.5 copies.",
  ...
  linearRanking: {
    ...
    latex:
      "P(i) = \\frac{1}{\\mu}\\left[s - (s-1)\\frac{rank(i)-1}{\\mu-1}\\right]",
    latexDisplay:
      "$$P(i) = \\frac{1}{\\mu}\\left[s - (s-1)\\frac{rank(i)-1}{\\mu-1}\\right]$$",
    variables: {
    ...
    plainEnglish:
      "Selection probability depends on RANK, not fitness value. Parameter s controls selection pressure: s=1 means uniform random, s=2 means best has twice the average probability.",
    whenToUse:
      "When you want selection pressure independent of fitness scaling. More robust than FPS.",
    example:
      "With μ=10 and s=2: Best (rank 10) gets P=0.2, Worst (rank 1) gets P=0. Middle (rank 5) gets P=0.1.",
  ...
  schemaTheorem: {
    ...
    latex:
      "m(H,t+1) \\geq m(H,t) \\cdot \\frac{f(H)}{\\langle f \\rangle} \\cdot \\left[1 - p_c \\cdot \\frac{\\delta(H)}{l-1}\\right] \\cdot (1-p_m)^{o(H)}",
    latexDisplay:
      "$$m(H,t+1) \\geq m(H,t) \\cdot \\frac{f(H)}{\\langle f \\rangle} \\cdot \\left[1 - p_c \\cdot \\frac{\\delta(H)}{l-1}\\right] \\cdot (1-p_m)^{o(H)}$$",
    variables: {
    ...
    plainEnglish:
      "Schemata with above-average fitness, short defining length, and low order will increase in frequency. The theorem gives a LOWER BOUND on growth because it ignores constructive effects.",
    whenToUse:
      "For understanding GA behavior, designing representations (keep related genes close), explaining why short fit schemata grow.",
    example:
      "Schema 1##0 with above-average fitness, short δ=3, low o=2 will grow faster than schema 1#####0 with long δ=6.",
  ...
  schemaDisruptionXover: {
    ...
    plainEnglish:
      "A schema is more likely to survive crossover if it has a short defining length (genes close together). The formula assumes 1-point crossover.",
    whenToUse:
      "When analyzing how crossover affects schema propagation, or when designing representations.",
    example:
      "Schema 1#0 (δ=2) in chromosome of length 10 with p_c=0.8: P_survive = 1 - 0.8×2/9 = 0.82",
  ...
  schemaDisruptionMutation: {
    ...
    latexDisplay:
      "$$P_{survive} = (1-p_m)^{o(H)} \\approx 1 - o(H) \\cdot p_m$$",
    variables: {
    ...
    plainEnglish:
      "A schema is more likely to survive mutation if it has low order (few defined positions). The approximation works when mutation probability is small.",
    whenToUse:
      "When analyzing mutation's effect on schemata, or when setting mutation rates.",
  ...
  tournamentExpected: {
    ...
    latex:
      "\\hat{s}(f_i) = N \\cdot \\left[\\left(\\frac{C(f_i)}{N}\\right)^t - \\left(\\frac{C(f_{i-1})}{N}\\right)^t\\right]",
    latexDisplay:
      "$$\\hat{s}(f_i) = N \\cdot \\left[\\left(\\frac{C(f_i)}{N}\\right)^t - \\left(\\frac{C(f_{i-1})}{N}\\right)^t\\right]$$",
    variables: {
    ...
    plainEnglish:
      "The expected number of individuals with a given fitness after tournament selection. Higher tournament size (t) increases selection pressure on high-fitness individuals.",
    whenToUse:
      "For analyzing tournament selection pressure and understanding how t affects selection.",
    example:
      "t=1: ŝ(f_i) = s(f_i), distribution unchanged (random selection). As t increases, distribution shifts toward higher fitness.",
  ...
  tournamentConcatenation: {
    ...
    latexDisplay:
      "$$\\Omega_{t_2}(\\Omega_{t_1}(s)) = \\Omega_{t_1 \\cdot t_2}(s)$$",
    variables: {
    ...
    plainEnglish:
      "Applying tournament selection with size t1, then with size t2, is equivalent to applying tournament selection once with size t1×t2. Selection pressures multiply.",
    whenToUse:
      "When analyzing multiple rounds of selection or understanding cumulative selection effects.",
    example:
      "Two rounds of binary tournament (t=2 twice) equals one round of t=4 tournament.",
  ...
  reproductionRate: {
    ...
    plainEnglish:
      "The reproduction rate tells us whether a fitness class will grow or shrink under selection. Values above 1 mean growth, below 1 mean shrinkage.",
    whenToUse:
      "For understanding which fitness levels gain or lose representation under selection.",
    example:
      "If s(f_i)=10 before and ŝ(f_i)=15 after selection, R=1.5 meaning 50% growth.",
  ...
  selectionIntensity: {
    ...
    plainEnglish:
      "Selection intensity measures how much selection improves the mean, in units of standard deviations. Higher I means stronger selection pressure.",
    whenToUse:
      "For comparing selection strength across different methods or populations, predicting convergence speed.",
    example:
      "I=1 means selection increases mean by one standard deviation. Truncation selection (top 10%) gives I≈1.76.",
  ...
  lossOfDiversity: {
    ...
    plainEnglish:
      "The fraction of the population that doesn't get selected at all. Higher selection pressure leads to more diversity loss.",
    whenToUse:
      "For analyzing premature convergence risk and choosing appropriate selection pressure.",
    example:
      "With binary tournament, if 50% have below-median fitness, P_d = 0.5² = 0.25 of population never selected.",
  ...
  varianceTheorem: {
    ...
    latexDisplay:
      "$$Var(\\hat{s}(f_i)) = N \\cdot p(f_i) \\cdot (1 - p(f_i))$$",
    variables: {
    ...
    plainEnglish:
      "The variance in how many individuals of a given fitness get selected. Roulette wheel has high variance; SUS has lower variance.",
    whenToUse:
      "For comparing selection methods (roulette vs SUS), understanding selection reliability.",
    example:
      "If p=0.3 and N=100: Var = 100×0.3×0.7 = 21, so std dev ≈ 4.6 individuals.",
  ...
  oneMaxFitness: {
    ...
    plainEnglish:
      "The OneMax problem counts the number of 1-bits in a binary string. It's a simple test problem where the optimal solution is all 1s.",
    whenToUse:
      "As a benchmark for testing EA performance, understanding basic EA behavior.",
  ...
  eightQueensFitness: {
    ...
    plainEnglish:
      "For the 8-Queens problem, fitness counts non-attacking queen pairs. Maximum 28 means no queen attacks another.",
