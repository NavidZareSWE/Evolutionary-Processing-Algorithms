...
export default function SelectionComparison() {
  ...
  const generatePopulation = useCallback(() => {
    ...
    const newIndividuals: Individual[] = fitnesses.map((fitness, i) => {
      ...
      const rankProbability = (1/n) * (s - (s - 1) * (2 * (rank - 1)) / (n - 1));
  ...
  const runSelections = useCallback(() => {
    ...
    for (let i = 0; i < numSelections; i++) {
      ...
      const winner = participants.reduce((best, idx) => 
        newIndividuals[idx].fitness > newIndividuals[best].fitness ? idx : best, 
        participants[0]
      );
  ...
  const maxSelections = Math.max(
    ...individuals.map(ind => Math.max(ind.selected.fps, ind.selected.rank, ind.selected.tournament)),
    1
  );
    ...
    <div className="visualization-container">
      <div className="viz-header">
        ...
        <p className="viz-description">
          Compare how FPS (Fitness Proportionate), Rank-based, and Tournament selection 
          distribute selections. Notice how FPS over-selects the fittest while Tournament 
          uses only comparisons (CORRECT fitness).
        </p>
      </div>
      ...
      <div className="viz-settings inline">
        <div className="setting-row">
          ...
          <input 
            type="range" min="2" max="5" value={tournamentSize}
        ...
        <div className="setting-row">
          ...
          <input 
            type="range" min="10" max="20" value={rankingPressure * 10}
        ...
        <div className="setting-row">
          ...
          <input 
            type="range" min="50" max="500" step="50" value={numSelections}
        ...
        {[...individuals].reverse().map((ind) => (
          <div key={ind.id} className="comparison-row">
            <span className="col-fitness">
              ...
              <div className="fitness-bar" style={{ width: `${ind.fitness}%` }} />
            </span>
            
            <span className="col-method fps">
              <span className="prob">P={(ind.fpsProbability * 100).toFixed(1)}%</span>
              <span className="count">{hasRun ? ind.selected.fps : '-'}</span>
              {hasRun && (
                <div className="selection-bar fps" 
                  style={{ width: `${(ind.selected.fps / maxSelections) * 100}%` }} />
              )}
            </span>
            
            <span className="col-method rank">
              <span className="prob">P={(ind.rankProbability * 100).toFixed(1)}%</span>
              <span className="count">{hasRun ? ind.selected.rank : '-'}</span>
              {hasRun && (
                <div className="selection-bar rank" 
                  style={{ width: `${(ind.selected.rank / maxSelections) * 100}%` }} />
              )}
            </span>
            
            <span className="col-method tournament">
              <span className="count">{hasRun ? ind.selected.tournament : '-'}</span>
              {hasRun && (
                <div className="selection-bar tournament" 
                  style={{ width: `${(ind.selected.tournament / maxSelections) * 100}%` }} />
              )}
            </span>
          </div>
        ))}
        ...
        <div className="selection-insights">
          ...
          <ul>
            <li>
              ...
              {((individuals[9].fpsProbability / individuals[0].fpsProbability)).toFixed(1)}x 
              higher probability than fitness 10. This uses EXACT fitness values!
            </li>
            <li>
              <strong>Rank-based:</strong> More uniform distribution. Only uses RANKINGS, 
              not exact values. Adding 1000 to all fitness values wouldn't change anything.
            </li>
            <li>
              <strong>Tournament:</strong> Only compares "who is better?" (CORRECT fitness). 
              Higher k = more selection pressure. No need to know exact fitness values!
            </li>
          </ul>
          <div className="insight-highlight">
            <strong>Professor's Key Point:</strong> We know CORRECT fitness (rankings), 
            not EXACT fitness (values). Tournament selection is robust because it only 
            uses comparisons!
          </div>
        </div>
      )}
