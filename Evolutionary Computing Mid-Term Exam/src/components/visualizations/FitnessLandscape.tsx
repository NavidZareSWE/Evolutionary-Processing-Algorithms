...
import { Mountain, Play, Pause, RotateCcw, ZoomIn, ZoomOut } from 'lucide-react';
...
export default function FitnessLandscape() {
  ...
  const fitnessFunction = useCallback((x: number, y: number): number => {
    switch (landscapeType) {
      ...
      case 'deceptive':
        ...
        const centerAttractor = 0.8 * Math.exp(-((x - 50) ** 2 + (y - 50) ** 2) / 800);
        const globalOptimum = 1.0 * Math.exp(-((x - 90) ** 2 + (y - 90) ** 2) / 100);
  ...
  const initPopulation = useCallback(() => {
    ...
    setGeneration(0);
    setBestPoint(newPop.reduce((best, p) => p.fitness > best.fitness ? p : best, newPop[0]));
    setIsRunning(false);
  }, [fitnessFunction]);
  ...
  const runGeneration = useCallback(() => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const parent1 = tournament.reduce((best, p) => p.fitness > best.fitness ? p : best);
      
      const tournament2 = [];
      for (let t = 0; t < 3; t++) {
        tournament2.push(population[Math.floor(Math.random() * population.length)]);
      }
      const parent2 = tournament2.reduce((best, p) => p.fitness > best.fitness ? p : best);
      ...
      newPop.push({ x: childX, y: childY, fitness: fitnessFunction(childX, childY) });
    }
    
    // Elitism - keep best
    const currentBest = population.reduce((best, p) => p.fitness > best.fitness ? p : best);
    newPop[0] = { ...currentBest };
    
    setPopulation(newPop);
    setGeneration(g => g + 1);
    
    const newBest = newPop.reduce((best, p) => p.fitness > best.fitness ? p : best);
  ...
  useEffect(() => {
    ...
    population.forEach((point, i) => {
      ...
      ctx.fillStyle = bestPoint && point.x === bestPoint.x && point.y === bestPoint.y 
    ...
    let optX = 50, optY = 50;
    if (landscapeType === 'multimodal') { optX = 75; optY = 75; }
    if (landscapeType === 'deceptive') { optX = 90; optY = 90; }
    if (landscapeType === 'rugged') { optX = 70; optY = 70; }
  ...
  }, [population, fitnessFunction, showContours, zoom, bestPoint, landscapeType]);
    ...
    <div className="visualization-container">
      <div className="viz-header">
        ...
        <p className="viz-description">
          ...
          Blue dots are individuals, gold is the best found. Green circle marks the global optimum.
        </p>
      </div>
      ...
      <div className="viz-controls">
        ...
        <button className="viz-btn" onClick={() => setZoom(z => Math.min(2, z + 0.25))}>
          <ZoomIn size={16} />
        </button>
        <button className="viz-btn" onClick={() => setZoom(z => Math.max(0.5, z - 0.25))}>
          <ZoomOut size={16} />
        </button>
      </div>
      ...
      <div className="landscape-display">
        ...
        <div className="landscape-stats">
          ...
          <div className="stat-item">
            ...
            <span className="stat-value">{bestPoint?.fitness.toFixed(4) || '-'}</span>
          </div>
          <div className="stat-item">
              ...
              {population.length > 0 
                ? (Math.sqrt(
                    population.reduce((sum, p) => 
                      sum + (p.x - population[0].x) ** 2 + (p.y - population[0].y) ** 2, 0
                    ) / population.length
                  )).toFixed(1)
                : '-'}
            </span>
      ...
      <div className="landscape-explanation">
        {landscapeType === 'unimodal' && (
          <div className="explanation-box success">
            ...
            <p>Single global optimum at center. Easy for GA—any hill-climbing works!</p>
        ...
        {landscapeType === 'multimodal' && (
          <div className="explanation-box warning">
            ...
            <p>Multiple peaks of varying heights. GA must maintain diversity to find the best.</p>
        ...
        {landscapeType === 'deceptive' && (
          <div className="explanation-box danger">
            ...
            <p>
              The center attracts the population, but the global optimum is in the corner!
              Building blocks point away from the true optimum.
            </p>
            <p><strong>This is GA-hard!</strong> Notice how the population gets stuck.</p>
          </div>
        )}
        {landscapeType === 'rugged' && (
          <div className="explanation-box warning">
            <h4>Rugged (NK-like) Landscape</h4>
            <p>Many local optima with epistatic interactions between variables.</p>
