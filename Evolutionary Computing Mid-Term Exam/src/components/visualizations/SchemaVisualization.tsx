...
export default function SchemaVisualization() {
  ...
  const analysis = useMemo((): SchemaAnalysis => {
    ...
    const fixedPositions = schema
      .split("")
      .map((c, i) => (c !== "#" ? i : -1))
      .filter((i) => i >= 0);
    const definingLength =
    ...
    const avgPopFitness =
      populationFitness.reduce((a, b) => a + b, 0) / populationFitness.length;
    ...
    const crossoverSurvival =
      1 - (crossoverRate * definingLength) / (chromosomeLength - 1);
    ...
    const expectedGrowth =
      selectionFactor * crossoverSurvival * mutationSurvival;
    ...
    <div className="visualization-container">
      <div className="viz-header">
        ...
        <p className="viz-description">
          ...
          The theorem predicts that short, low-order, above-average schemata
          grow exponentially.
        </p>
      </div>
      ...
      <div className="schema-display">
        ...
        <div className="schema-visual">
          {schema.split("").map((char, i) => (
            <span
              key={i}
              className={`schema-char ${char === "#" ? "wildcard" : "fixed"}`}
            >
              {char}
            </span>
          ))}
        ...
        <p className="schema-explanation">
          This schema matches <strong>{analysis.matchingCount}</strong> possible
          chromosomes ({matchingChromosomes.slice(0, 4).join(", ")}
          {matchingChromosomes.length > 4 ? "..." : ""})
        </p>
      </div>
      ...
      <div className="viz-settings inline">
        <div className="setting-row">
          <label>
            Crossover Rate (p_c): {(crossoverRate * 100).toFixed(0)}%
          </label>
          <input
            type="range"
            min="0"
        ...
        <div className="properties-grid">
          ...
          <div className="property-card">
            ...
            <span className="property-value">
              2^{chromosomeLength - analysis.order} = {analysis.matchingCount}
            </span>
            <span className="property-desc">
              Chromosomes in this hyperplane
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="schema-theorem-calc">
        ...
        <div className="theorem-formula">
          <div className="formula-line">
            m(H, t+1) ≥ m(H, t) ×{" "}
            <span className="factor selection">[f(H)/⟨f⟩]</span> ×
        ...
        <div className="factors-breakdown">
          <div className="factor-card selection">
            ...
            <div className="factor-calc">
              <span>
                f(H) / ⟨f⟩ = {analysis.avgFitness.toFixed(2)} /{" "}
                {avgPopFitness.toFixed(2)}
              </span>
              <span className="factor-result">
                = {analysis.selectionFactor.toFixed(3)}
              </span>
            </div>
          ...
          <div className="factor-card crossover">
            ...
            <div className="factor-calc">
              <span>
                1 - {crossoverRate.toFixed(2)} × {analysis.definingLength} /{" "}
                {chromosomeLength - 1}
              </span>
              <span className="factor-result">
                = {analysis.crossoverSurvival.toFixed(3)}
              </span>
            </div>
            <p className="factor-interpretation">
          ...
          <div className="factor-card mutation">
            ...
            <div className="factor-calc">
              <span>
                (1 - {mutationRate.toFixed(3)})^{analysis.order}
              </span>
              <span className="factor-result">
                = {analysis.mutationSurvival.toFixed(3)}
              </span>
            </div>
            <p className="factor-interpretation">
              {analysis.order <= 2
        ...
        <div className="total-growth">
          ...
          <div className="growth-calc">
            ...
            <span
              className={`growth-result ${analysis.expectedGrowth > 1 ? "positive" : "negative"}`}
            >
      ...
      <div className="building-block-insight">
        ...
        <p>
          <strong>Short, low-order, above-average schemata</strong> are called{" "}
          <em>building blocks</em>. The GA works by discovering these building
          blocks and combining them into complete solutions.
        </p>
        <div className="bb-checklist">
          <div
            className={`bb-check ${analysis.definingLength <= 2 ? "pass" : "fail"}`}
          >
            {analysis.definingLength <= 2 ? "✓" : "✗"} Short defining length (δ
            ≤ 2): δ = {analysis.definingLength}
          </div>
          <div className={`bb-check ${analysis.order <= 3 ? "pass" : "fail"}`}>
            {analysis.order <= 3 ? "✓" : "✗"} Low order (o ≤ 3): o ={" "}
            {analysis.order}
          </div>
          <div
            className={`bb-check ${analysis.selectionFactor > 1 ? "pass" : "fail"}`}
          >
            {analysis.selectionFactor > 1 ? "✓" : "✗"} Above-average fitness:{" "}
            {analysis.selectionFactor.toFixed(2)} {">"} 1
          </div>
        </div>
        <p
          className={`bb-verdict ${analysis.definingLength <= 2 && analysis.order <= 3 && analysis.selectionFactor > 1 ? "is-bb" : "not-bb"}`}
        >
          {analysis.definingLength <= 2 &&
          analysis.order <= 3 &&
          analysis.selectionFactor > 1
